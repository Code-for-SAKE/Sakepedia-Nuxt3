...
const schema = object({
    breweryId: string().required('法人番号は必須です').max(13, '13桁以内で入力してください'),
    name: string()
        .required('名前は必須です'),
    address: string(),
    prefecture: string(),
        ...
        <div class="grid grid-cols-6 gap-4">
            <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
                ...
                <UFormGroup label="住所" name="address" required="true">
                    ...
                    <small>都道府県、緯度経度は住所から自動的に登録されます。
                        <a href="https://geolonia.com/">Geolonia</a>の<a
                            href="https://github.com/geolonia/normalize-japanese-addresses">住所正規化ライブラリ</a>を利用しています。<br />
                        正確な緯度経度がわかる場合は入力してください。
                    </small>
                </UFormGroup>
                <UFormGroup label="都道府県" name="prefecture">
                    <UInputMenu id="prefecture" v-model="state.prefecture" :options="prefectures" value-attribute="id"
